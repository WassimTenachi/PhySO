...
def save_latex_equations(equations: List[str],
                         filename: str,
                         fontsize: int = 20,
                         figsize: tuple = (14, 10),
                         dpi: int = 300,
                         usetex: bool = True):
    """
    Save LaTeX equations to an image file.

    Parameters:
    -----------
    equations : List[str]
        List of 5 LaTeX equation strings (e.g., [r'$a$', r'$b$', ...])
    filename : str
        Output filename (e.g., 'equations.png')
    fontsize : int, optional
        Font size for equations (default: 20)
    figsize : tuple, optional
        Figure size in inches (default: (14, 10))
    dpi : int, optional
        Output resolution (default: 300)
    usetex : bool, optional
        Whether to use LaTeX rendering (default: True)
    ...
    try:
        ...
        # Validate input
        if len(equations) != 5:
            raise ValueError("Exactly 5 equations required")

        # Display equations with vertical spacing
        for i, eq in enumerate(equations, 1):
            plt.text(0.5, 0.9 - 0.15 * i, "$"+eq+"$",
                     fontsize=fontsize,
                     ha='center',
                     va='center')
        ...
        plt.close()
        print(f"Successfully saved equations to {filename}")

    except Exception as e:
        plt.close()
        raise RuntimeError(f"Error rendering equations: {str(e)}")


PROPERTIES_TO_CHECK = ["enclosed mass", "potential",] #"radial velocity dispersion"]
#PROPERTIES_TO_CHECK = ["enclosed mass", "potential", "radial velocity dispersion"]
PROPERTIES_TO_PLOT = ["Density", "Enclosed Mass", "Circular Velocity", "Potential",]# "Radial Velocity Dispersion"]
...
def my_RewardsComputer(programs,
    ...
    for i in range (programs.batch_size):
        ...
        if mask_valid[i]:
            try:
                j=i+1
                if j==4094 or j == 5587 or j==5880 or j==6788 or j==6915 or j==7658 or j==8190 or j==8235 or j==9140:
                    raise ValueError("Sympy mess.")
                if j < -1:
                    raise ValueError("tmp test fix.")
                ...
                results_df = ap.check_analytical_properties(density_profile_str=prog_str,
                                                            free_consts_names=["c0", "rho0", "Rs0", "Rs1"],
                                                            free_consts_vals={},
                                                            num_check=False,
                                                            verbose=True,)
                print(" -> Checking done")
                ...
                if is_ok:
                    # Plot stuff
                    plot_df = results_df[results_df["Property"].str.lower().isin([p.lower() for p in PROPERTIES_TO_PLOT])]
                    do_plot = True
                    plot_eqs = [sympy.latex(expr) for expr in plot_df["symb_result"]] + ["-----------"]
                    for expr in plot_eqs:
                        if len(expr) < 4: # nan or 0
                            warnings.warn("Skipping plotting of expr %s, too short."%(expr))
                            do_plot = False
                    if do_plot:
                        save_latex_equations(plot_eqs, "equations_%i.png"%(n_has_prop))
                        n_has_prop += 1
            except:
                warnings.warn("Error while checking analytical properties of program %i, skipping it."%(i))
                is_ok = False
        else:
        ...
        #todo: check not nan
        print("Found %i programs satisfying analytical properties so far."%(n_has_prop))
...
# Hack here
my_config["learning_config"]["batch_size"] = int(1e4) # tmp
...
N_EPOCHS          = int(1e99) # int(1e99)  #int(1e99) # tmp 1 #
