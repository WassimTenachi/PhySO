...
# ---------------------------------------------------- SCRIPT ARGS -----------------------------------------------------
parser = argparse.ArgumentParser (description     = "Runs a z_reion SR job.", # z_reion SPECIFIC
                                  formatter_class = argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument("-zt", "--ztask", default = 'z_mid', # z_reion SPECIFIC
                    help = "Which task to run: z_asy, z_dur or z_mid.")
...
if __name__ == '__main__':
    ...
    df = pd.read_csv('../data/params_training_data.csv') # z_reion SPECIFIC
    # endregion

    # region ## Run name and paths
    # Paths
    RUN_NAME = ("ZREION_SR_%s_xe%i_fp%d_lloc%d_lscale%d_s%d" % (ZTASK, int(X_ESSENTIAL_ONLY), N_FREE_PARAMS, LENGTH_LOC_ARG, LENGTH_SCALE_ARG, SEED)) # z_reion SPECIFIC
