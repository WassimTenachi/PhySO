...
RUNS_PATH = "/Users/wtenachi/Documents/ASTRO_research/projects/reionization-sr/season4/run-results/XHI_SR-RUNS-APLHA/"
...
# list folders starting with "XHI_SR" # xHI specific
run_folders = [f for f in os.listdir(RUNS_PATH) if f.startswith("XHI_SR")]  # xHI specific
...
path_data_train = os.path.join(PATH_DATA, "xHI_training_data.csv") # xHI specific
path_data_test  = os.path.join(PATH_DATA, "xHI_test_data.csv")     # xHI specific
df_train = pd.read_csv(path_data_train)
df_test  = pd.read_csv(path_data_test)

weight_func = lambda df: 0.5 * (1 - np.tanh(10 * (df["z"] - 20)))  # (n_samples,)  # xHI specific
...
if not found_all_exprs:
    for index, row in all_exprs_df.iterrows():
        ...
        for mode in ["train", "test"]:
            ...
            weights = weight_func(df)                                     # (n_samples,)       # xHI specific

            X = df[vars_used].to_numpy()                                  # (n_samples, n_dim)
            y_target = df["xHI"].to_numpy()                               # (n_samples,)       # xHI specific
            y_pred   = expr(torch.tensor(X.T)).cpu().detach().numpy()     # (n_samples,)

            # R2
            r2 = metrics_utils.r2(y_target=y_target, y_pred=y_pred, weights=weights)   # xHI specific
            all_exprs_df.at[index, f"R2_{mode}"] = r2

            # MAE
            MAE = np.average(np.abs(y_target-y_pred), weights=weights)                 # xHI specific
...
y_target    = df_test["xHI"].to_numpy()                                    # (n_samples,)       # xHI specific
...
astro_cols = [
    "z", "OMm", "OMb","sigma_8", "n_s",
    "F_STAR10", "F_ESC10", "ALPHA_STAR", "ALPHA_ESC",
    "M_TURN", "R_BUBBLE_MAX", "L_X", "t_STAR"
]  # xHI specific
...
baseline_str = "((((((((((exp(M_TURN * (L_X * ((((exp(M_TURN * (-74.936905 * (((safe_log(ALPHA_STAR) + (z * (ALPHA_ESC + -0.3475463))) * ((0.63973224 * OMb) - sigma_8)) ^ -3.7277033))) + sigma_8) * (((exp((((z * ((0.7161328 - (ALPHA_ESC + (ALPHA_STAR + -0.3527866))) * safe_sin(M_TURN))) ^ -3.7277043) * z) * -74.936905) + sigma_8) * -2.0090358) * (((M_TURN - (sigma_8 + (n_s ^ R_BUBBLE_MAX))) * ((((safe_sin(ALPHA_STAR) + z) - sigma_8) * 0.031346083) * z)) ^ -3.724665))) * (((safe_log(R_BUBBLE_MAX) ^ -0.501449) * 0.033237148) ^ -3.307042)) * (((((z * M_TURN) * 0.062762536) ^ F_STAR10) * ((OMm + (sigma_8 ^ 0.5483802)) - (F_ESC10 * -0.8242562))) ^ 2.6457334)))) ^ 0.5095854) ^ sigma_8) ^ OMm) ^ OMm) ^ n_s) ^ n_s) ^ n_s) ^ n_s) ^ n_s) ^ n_s) ^ n_s"

# ---- Metrics : accuracy ----
baseline_func = make_callable_equation(baseline_str, astro_cols)

weights = weight_func(df_test)                               # (n_samples,)        # xHI specific

X = df_test[astro_cols].to_numpy()                           # (n_samples, n_dim)  # xHI specific
y_target    = df_test["xHI"].to_numpy()                      # (n_samples,)        # xHI specific
y_pred      = np.array([baseline_func(row) for row in X])    # (n_samples,)

baseline_mae_test = np.average(np.abs(y_target-y_pred), weights=weights)                  # xHI specific
baseline_r2_test  = metrics_utils.r2(y_target=y_target, y_pred=y_pred, weights=weights)   # xHI specific

print("Baseline MAE:", baseline_mae_test)
print("Baseline R2 :", baseline_r2_test)
