...
# ---------------------------------------------------- SCRIPT ARGS -----------------------------------------------------
parser = argparse.ArgumentParser (description     = "Runs an xHI SR job.", # xHI SPECIFIC
...
if __name__ == '__main__':
    ...
    df = pd.read_csv('../data/xHI_training_data.csv') # xHI SPECIFIC
    # Subsample only 5k lines
    df = df.sample(n=5000, random_state=SEED).reset_index(drop=True) # xHI SPECIFIC

    # endregion

    # region ## Run name and paths
    # Paths
    RUN_NAME = ("XHI_SR_xe%i_fp%d_lloc%d_lscale%d_s%d" % (int(X_ESSENTIAL_ONLY), N_FREE_PARAMS, LENGTH_LOC_ARG, LENGTH_SCALE_ARG, SEED)) # xHI SPECIFIC
    ...
    X_names = ['z', 'OMm', 'OMb', 'h', 'sigma_8', 'n_s', 'F_STAR10', 'F_ESC10', 'ALPHA_STAR', 'ALPHA_ESC', 'M_TURN', 'L_X', 't_STAR', 'R_BUBBLE_MAX',] # xHI SPECIFIC
    X_names_essential = ['z','ALPHA_ESC','F_ESC10','M_TURN','F_STAR10',] # xHI SPECIFIC
    if X_ESSENTIAL_ONLY:
        X_names = X_names_essential

    n_dim = len(X_names)
    X = df[X_names].to_numpy().T                # (n_dim, n_samples)
    y_name = 'xHI' # xHI SPECIFIC
    ...
    OP_NAMES = ["mul", "add", "sub", "div", "inv", "neg", "n2", "sqrt", "exp", "log", "sin", "tanh"] # xHI SPECIFIC
    # pow
