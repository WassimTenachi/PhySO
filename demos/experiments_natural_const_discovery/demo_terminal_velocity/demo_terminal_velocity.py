...
if __name__ == '__main__':
    ...
    # Data points
    m = np.random.uniform(1, 10, data_size)  # kg
    rho = np.random.uniform(1, 6, data_size)  # kg/m3
    A = np.random.uniform(1, 5, data_size)  # m2
    X_array = np.stack((m, rho, A), axis=0)
    ...
    # Normalize by
    norm = 1. / 5  # np.sqrt(9.8/0.47)# 1./2

    y_array = norm * np.sqrt(2 * m * g / (rho * A * Cd))
    ...
    args_make_tokens = {
                    ...
                    # input variables
                    "input_var_ids"        : {"m" : 0         , "rho" : 1         , "A" : 2         , },
                    "input_var_units"      : {"m" : [0, 0, 1] , "rho" : [-3, 0, 1], "A" : [2, 0, 0] , },
                    "input_var_complexity" : {"m" : 1.        , "rho" : 1.        , "A" : 1.        , },
                    # constants
                    #"constants"            : {"1" : const1    , },
                    #"constants_units"      : {"1" : [0, 0, 0] , },
                    #"constants_complexity" : {"1" : 1.        , },
                    # free constants
                    "free_constants"            : {"CA"              , "Cvt"               ,  "Cm"              , "Crho"              , },
                    "free_constants_init_val"   : {"CA" : 1.         , "Cvt" : 1.          ,  "Cm" : 1.         , "Crho" : 1.         , },
                    "free_constants_units"      : {"CA" : [2, 0, 0]  , "Cvt" : [1, -1, 0]  ,  "Cm" : [0, 0, 1]  , "Crho" : [-3, 0, 1] , },
                    "free_constants_complexity" : {"CA" : 1.         , "Cvt" : 1.          ,  "Cm" : 1.         , "Crho" : 1.         , },
                        }
    ...
    priors_config  = [
                    ...
                    #("OccurrencesPrior", {"targets" : ["1", "Cm", "Crho", "CA", "Cvt",] , "max" : [3, 2, 2, 2, 2,] }),
                     ]
