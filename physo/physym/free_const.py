...
class FreeConstantsTable:
    ...
    def flatten_like_data (self, n_samples_per_dataset):
        ...
        spe_const_flatten = self.spe_values.repeat_interleave(torch.tensor(n_samples_per_dataset), dim=-1)         # (batch_size, n_spe_free_const, n_all_samples)
    ...
    def df (self):
        ...
        values = np.reshape(values, newshape=(self.batch_size, self.n_spe_free_const*self.n_realizations))   # (batch_size, n_spe_free_const*n_realizations)
