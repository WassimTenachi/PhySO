...
def assess_r2_in_pareto (pareto_df, Feynman_pb):
    """
    Computes R2 value of the best Pareto front expression.
    Parameters
    ----------
    pareto_df : pd.DataFrame
        Pareto front dataframe generated by PhySO.
    ...
    pareto_expressions = load_pareto_expressions(pareto_df            = pareto_df,
                                                 sympy_X_symbols_dict = Feynman_pb.sympy_X_symbols_dict, )
    # Generate test data
    X, y = pb.generate_data_points(int(1e4))
...
# Results df settings
column_names = ["Eq Nb"] + ["Trial %i"%(i) for i in range(N_TRIALS)] + ["Recovery Rate", "R2", "# evaluations", "# started", "# finished"]
column_dtypes = {"Eq Nb": int, "Recovery Rate" : float, "R2" : float, "# evaluations" : int, "# started" : int, "# finished" : int,}
    ...
    if pb.eq_filename not in EXCLUDED_IN_SRBENCH_EQS_FILENAMES:
        ...
        for i_trial in range (N_TRIALS):
            ...
            try:
                ...
                # Get r2
                r2 = assess_r2_in_pareto(pareto_df = pareto_data, Feynman_pb = pb)
                # Managing runs
                is_started    = True
        ...
        r2_pb      = np.sum(r2_list)            / N_TRIALS
        data = np.array([i_eq] + is_equivalent_list + [recov_rate, r2_pb, tot_evals, n_started, n_finished])[:, np.newaxis].transpose()
...
def compute_95_ci (data):
    ...
    return res
ci_95_rr = compute_95_ci(results_df["Recovery Rate"])
ci_95_r2 = compute_95_ci(results_df["R2"][np.abs(results_df["R2"]) < 1e1])


print("\n\n")
print("Total recovery rate    = %f %%"          %(100*total_recov_rate))
print("Recovery rate 95%% CI   = %f %%  - %f %%" %(100*ci_95_rr[0], 100*ci_95_rr[1]))
print("Total R2 coef          = %f"             %(total_R2))
print("R2 coef 95%% CI         = %f - %f "      %(ci_95_r2[0], ci_95_r2[1]))
